...
(async () => {
  ...
  if (!yontem) {
    throw new Error(
      'guncellemeler/guncelleme.json yok.\n' +
        'Gelistiriciden surum paketini (ZIP + guncelleme.json) alip\n' +
        'C:\\ckspaket\\guncellemeler\\ klasorune kopyalayin.'
    );
  }

  let guncellendi = false;

  if (yontem === 'yerel' || yontem === 'uzak') {
    console.log(yontem === 'yerel' ? 'Yontem: Yerel paket (guncellemeler/)' : 'Yontem: Uzak ZIP (GUNCELLEME_URL)');
