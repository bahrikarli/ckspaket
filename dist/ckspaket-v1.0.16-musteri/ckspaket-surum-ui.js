...
(function () {
  ...
  function surumCikButonuGoster() {
    ...
    btn.title = 'Müşteri paketi oluştur ve sürümü artır';
  ...
  async function surumCikBaslat() {
    ...
    modalGoster('Sürüm Çıkılıyor', 'Müşteri paketi hazırlanıyor…', 10);
    ...
    try {
      ...
      while (Date.now() - baslangic < 10 * 60 * 1000) {
        ...
        if (durum) {
          modalGoster('Sürüm Çıkılıyor', durum.mesaj || 'Paketleniyor…', durum.yuzde || 50);
          if (durum.bitti) {
            if (durum.hata) {
              modalHata(durum.mesaj || durum.hata);
            } else {
