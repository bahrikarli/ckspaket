...
// express.static asagida — kurum adi enjeksiyonlu / ve /anasayfa.html once kayit edilir
...
sql.connect(dbConfig).then(async (p) => {
  ...
  if (!CKSPAKET_MOD) process.exit(1);
  console.warn('[CKS Paket] Veritabani baglantisi yok — sunucu acik kalacak; SQL duzeltilene kadar giris calismaz.');
});
// Token kontrol
// ==========================================
...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, ckspaket: CKSPAKET_MOD, pid: process.pid });
});

// Tanımlamalardaki kurum adi — /api/kurum-genel + giris/anasayfa HTML enjeksiyonu
try {
  const { registerKurumSunucu } = require('./kurum-sunucu');
  registerKurumSunucu(app, { getPool, gercekKlasor, authenticateToken });
} catch (err) {
  console.warn('[kurum-sunucu] yuklenemedi:', err.message);
  const { kurumGenelOku } = require('./kurum-ayar');
...
try {
  ...
  console.warn('[paket-guncelleme] yuklenemedi:', err.message);
}

// TÜM SAYFALAR (/ ve /anasayfa.html kurum-sunucu.js tarafindan)
// HTML sayfalari — oturum istemcide (localStorage + /api/*); iframe/navigasyon Bearer tasimaz
app.get('/dashboard.html', (req, res) => res.sendFile(path.join(__dirname, 'dashboard.html')));
app.get('/mesajlar.html', (req, res) => res.sendFile(path.join(__dirname, 'mesajlar.html')));
app.get('/profil.html', (req, res) => res.sendFile(path.join(__dirname, 'profil.html')));
app.get('/personel-pdf-havuz.html', (req, res) => res.sendFile(path.join(__dirname, 'personel-pdf-havuz.html')));
app.get('/cks.html', (req, res) => res.sendFile(path.join(__dirname, 'cks.html')));
...
app.get('/mesai-yoklama.html', (req, res) => res.sendFile(path.join(__dirname, 'mesai-yoklama.html')));
app.get('/mesai-takip.html', (req, res) => res.sendFile(path.join(__dirname, 'mesai-takip.html')));
...
function pdfArsivKokAdaylari() {
    ...
    const adaylar = [birincil, path.join(gercekKlasorErken, 'taramalar')];
    const seen = new Set();
...
/** Belgenet tablosu bkullanici — giriş yapan personel (varsayılan BAHRİ KARLI kullanılmaz) */
async function belgenetPersonelAdiAl(req, dosyaAdi) {
    const gonderilen = String(req.body.kullanici || req.body.personel || '').trim();
    if (gonderilen) return gonderilen;
    const dosya = String(dosyaAdi || req.body.dosyadı || req.body.dosyadi || '').trim();
    if (dosya) {
        try {
            const pool = await getPool();
            const r = await pool.request()
                .input('dosya', sql.NVarChar, dosya.replace(/\.pdf$/i, ''))
                .query(`
                    SELECT TOP 1 kullanıcı FROM belgenet
                    WHERE dosyadı = @dosya OR dosyadı = @dosya + '.pdf'
                    ORDER BY kimlik DESC
                `);
            const taramaYapan = String(r.recordset[0]?.kullanıcı || '').trim();
            if (taramaYapan && taramaYapan !== '-') return taramaYapan;
        } catch (_) {}
    }
    return 'Bilinmiyor';
}
...
// ==========================================
        ...
        if (belgenetNo) {
            ...
            const personelAdi = await belgenetPersonelAdiAl(req, guncellenecekDosya);
...
(async () => {
  ...
  const lanHost = CKSPAKET_MOD ? '0.0.0.0' : lanBindHostBul();
  ...
  const sunucuHazirLog = async () => {
    ...
    const agIp = lanBindHostBul();
    if (agIp !== '127.0.0.1') {
      console.log(`Yerel ag (ofis) → http://${agIp}:${PORT}`);
    }
    if (CKSPAKET_MOD) {
    ...
    console.log(`Selektor mobil → http://${agIp}:${SELEKTOR_MOBIL_PORT}  (yalnizca tesis WiFi)`);
  ...
  try {
    await dinle(lanHost);
    if (!CKSPAKET_MOD && lanHost !== '127.0.0.1' && lanHost !== '0.0.0.0') {
    ...
    if (lanHost !== '127.0.0.1') {
      try {
        console.warn('[Ag] Ilk bind basarisiz, 127.0.0.1 deneniyor:', err.message);
        await dinle('127.0.0.1');
        await sunucuHazirLog();
        return;
      } catch (err2) {
        console.error('\n❌ Sunucu hatası:', err2.message);
        process.exit(1);
      }
    }
