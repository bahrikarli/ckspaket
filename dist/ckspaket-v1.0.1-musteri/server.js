...
app.get('/api/klasor-sayfa-istatistik', authenticateToken, async (req, res) => {
    try {
        ...
        const aralikSorgu = await pool.request().query(`
            WITH Evraklar AS (
                SELECT 
                    TRY_CAST(${SQL_DILEKCE_NO_DOSYADAN} AS INT) AS DilekceNo,
...
// ==========================================
/** belgenet dosyadından dilekçe no — 2027-1-... ve eski 1-... uyumlu */
const SQL_DILEKCE_NO_DOSYADAN = `CASE 
    WHEN dosyadı LIKE '[0-9][0-9][0-9][0-9]-%' AND CHARINDEX('-', dosyadı, 6) > 5
        THEN SUBSTRING(dosyadı, 6, CHARINDEX('-', dosyadı, 6) - 6)
...
app.post('/api/belgenet-ekle', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        let dosyadi = (adet === 0)
            ? `${aktifYil}-${dilekçeno}-${gecerliKimlikNo}`
            : `${aktifYil}-${dilekçeno}-EK-${adet}-${gecerliKimlikNo}`;
        //DNM
        ...
        await pool.request()
            ...
            .input('yil', sql.SmallInt, aktifYil)
